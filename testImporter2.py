...
class TestImporter2 (unittest.TestCase):
    ...
    def constructEmptyUDir(self, uDir):
        ...
        with open(idPath, 'r') as f:
            nodeID = f.read()
        self.assertEqual(65, len(nodeID))      # assumes this is a hex string
        self.assertEqual('\n', nodeID[64])     # 0-based
        nodeID = nodeID[:-1]
    ...
    def testImport(self):
        ...
        importer = Importer(self.srcDir, self.destDir,
                            'testImport ' + __version__)
        importer.doImportUDir()

        # verify that the files got there
        s2 = BlockingServer(self.destDir)
